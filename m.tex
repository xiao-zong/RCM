...
\end{eqnarray*}
where $\bm{x}=(x_1,\hdots,x_{2k-\ell})$ and $\bm{u}=(u_{i,j})_{i,j\in[2k-\ell], i\neq j}$. The right hand side is finite due to the assumption that $\E\left(\overline{S}^2\right)<\infty$.
...
Let $\widehat{S}^{(n)}_m$ be defined by replacing $f$ with $f_m$ in \eqref{Ustat1}. Then, by the bounded convergence theorem, we have 
\begin{eqnarray*}
...
Thus, we can apply the results from Proposition~\ref{normalU-1}.
\begin{eqnarray*}
    \widehat{S}^{(n)}&=&\sum_{(\hat{x}_1,\hdots,\hat{x}_k)\in\widehat{\eta}^k_{\neq}} f(x_1,\hdots,x_k, \tilde{m}^{(n)}(\hat{x}_1,\hdots,\hat{x}_k))\\
...
Note $f^{(n)}:\widehat{\mathbb{X}}^k\to \R$ is measurable and bounded. By virtue of Lemma~\ref{lma1}, we have, for eacn $\pi\in\overline{\Pi}_{\ge2}(i,i,j,j)$, 
\begin{eqnarray*}
...
&=&\int_{\mathbb{X}^{|\pi|}} \int_{\mathbb{M}^{|\pi|\times |\pi|}_{\neq}} h_{\pi}(x_1,\hdots,x_{|\pi|}, u) \, \mathbb{Q}_{\neq}^{|\pi|\times |\pi|}(du) \, \lambda^{|\pi|}(d(x_1,\hdots,x_|\pi|)),
\end{eqnarray*}
with $h_{\pi}:\mathbb{X}^{|\pi|}\times \mathbb{M}^{|\pi|\times |\pi|}_{\neq}\to\R$ defined by 
\begin{equation}
h_{\pi}(x_1,\hdots,x_{|\pi|}, u):=\left(\overline{f}_i\otimes\overline{f}_i\otimes\overline{f}_j\otimes\overline{f}_j\right)_{\pi}(x_1,\hdots,x_{|\pi|}, u),
\end{equation}
with 
\begin{eqnarray*}
    &&\overline{f}_{\ell}(x_1,\dots,x_{\ell},(u_{\imath,\jmath})_{1\le\imath\ne\jmath\le\ell})\\
    &:=&\int_{\mathbb{X}^{k-\ell}}\int_{\mathbb{M}^{k(k-1)-\ell(\ell-1)}}\left\vert f(x_1,\dots,x_{\ell},y_1,\dots,y_{k-\ell},(u_{\imath,\jmath})_{1\le\imath\ne\jmath\le k})\right\vert\mathbb{Q}^{k(k-1)-\ell(\ell-1)}(d\bm{v})\lambda^{k-\ell}(d(y_1,\dots,y_{k-\ell}))
\end{eqnarray*}
with $\bm{v}=\left((u_{\imath,\jmath})_{\substack{\ell<\imath\le k,\jmath\in[k]\\\imath\ne\jmath}},(u_{\imath,\jmath})_{\imath\in[\ell],\ell<\jmath\le k}\right)$ for $\ell=1,\dots,k$.
% \begin{eqnarray*}
