...
\title{\huge Functional approximation for $U$-statistics in the random connection model}
%\author{}
%\date{\today}
...
with various approaches being proposed to analyze the structure and asymptotic properties of these random graphs. In this note, we focus on various approximation results for counting statistics in RCM by incorporating the random connection model into the framework of Poisson $U$-statistics, see \cite{ReitznerSchulte13}, \cite{last4}, and \cite{schulte} for more details.

\section{Construction of the random connection model}

In \cite{last2}, the random connection model can be modelled in edge-marked Poisson processes. Alternatively, we can construct the random connection model using a Poisson point process with independent marks.

Let $\mathbf{Y}:=[0,1]^{\mathbb{N}\times\mathbb{N}}$ be the mark space and $\mathbb{Q}$ be the distribution of a double sequence of independent random variables uniformly distributed on $[0, 1]$. 
Let $\hat{\eta}$ be a $\mathbb{Q}$-marking of the Poisson process $\eta$. Known as the marking theorem, c.f. \cite[Themore~5.6]{last3}, $\hat{\eta}$ is a Poisson point process on the space $S:=\mathbb{R}^d \times \mathbf{Y}$ with intensity measure $\lambda_d \otimes \mathbb{Q}$, where $\lambda_d$ is the Lebesgue measure on $\mathbb{R}^d$. The points in $\hat{\eta}$ are of the form $(x,\mathbf{u})=(x,(u_{m,n})_{m,n=1}^\infty)$, where $x\in\mathbb{R}^d$ and $\mathbf{u}=(u_{m,n})_{m,n=1}^\infty$ is a double sequence of independent random variables uniformly distributed on $[0, 1]$.
% Then we consider an independent $\mathbb{Q}$-marking $\hat{\eta}$ of $\eta$, that is a Poisson point process on $\mathbb{R}^d\times \mathbf{Y}$, c.f. \cite[Theorem~5.6]{last3}. 

Let $\{A_i\}_{i\in\mathbb{N}}$ be a partition of $\mathbb{R}^d$ that consists of bounded Borel sets. 
Taking an arbitrary ordering on $\mathbb{R}^d$, for example the lexicographic order, denoted by $\prec$, we have the following. 
For any two points $(x,\mathbf{u}),(y,\mathbf{v})\in\hat{\eta}$,
with $\mathbf{u}=(u_{i,j})_{i,j=1}^\infty$ and $\mathbf{v}=(v_{i,j})_{i,j=1}^\infty$, satisfying $x\prec y$ and $y\in A_i$ is the $m$-th nearest point to $x$ in $A_i$, $m\ge1$, we define $W_{x,y} := u_{i,m}$. The two distinct points $x,y\in\eta$ are connected by an edge if and only if $W_{x,y} \leq g(y-x)$, where $g$ is the connection function. The resulting random graph with vertex set $\eta$ is a random connection model with connection function $g$. 
% Then, the random graph $G(\eta)$, that has vertex set $\eta$ and in which two distinct vertices are connected by an edge if and only if $W_{x,y}\leq g(x-y)$, has the law of the random connection model with connection function $g$ in $\mathbb{R}^d$.

\section{Poisson $U$-statistics}
Let $f_t:(\R^d)^k\times[0,1]^{k(k-1)/2}\to\R$ be a measurable function, $t\ge1$. We say $f_t$ is symmetric if for every permutation $\sigma$ of $[k]:=\{1,2,\dots,k\}$, it holds that 
$$f_t\Big(x_1,\ldots,x_k,(w_{i,j})\Big)=f_t\Big(x_{\sigma(1)},\ldots,x_{\sigma(k)},(w_{\sigma(i),\sigma(j)})\Big).$$
Using the above notation, define
 ...
 \end{align}
 measurable function and the sum is taken over all $k$-tuples of pairwise distinct points. Note that $S$ is not a Poisson $U$-statistics, as the weight $W_{x_i,x_j}$ depend on other points besides $x_i$ and $x_j$.

Instead, we can consider the following structure. For two points $\hat{x},\hat{y}\in\hat{\eta}$ with $\hat{x}=(x,\mathbf{u})=(x,(u_{i,l})_{i,l\in\mathbb{N}})$ and $\hat{y}=(y,\mathbf{v})=(y,(v_{i,l})_{i,l\in\mathbb{N}})$ in $\hat{\eta}$ such that $x\prec y$ and $y\in A_i$, define $\hat{W}_{\hat{x},\hat{y}} := u_{i,1}$, and
\begin{eqnarray}
\hat{S}_t&:=& \frac{1}{k!}\sum_{(\hat{x}_1, \ldots, \hat{x}_k) \in \hat{\eta}^k_{\neq}} f_t \Big( {x}_1, \ldots, {x}_k, (\hat{W}_{\hat{x}_i,\hat{x}_j}) \Big) \\
&=&  \frac1{k!}\sum_{(\hat{x}_1, \ldots, \hat{x}_k) \in \hat{\eta}^k_{\neq}} \hat{f}_t ( \hat{x}_1, \ldots, \hat{x}_k ),
\end{eqnarray}
which on the other hand is a Poisson $U$-statistics. Now, choose the partition $\{A_i\}_{i\in\mathbb{N}}$ such that $\lambda_d (A_i) \textcolor{red}{\le} \frac{\varepsilon}{i}$ for every $i \in \mathbb{N}$, where $\varepsilon>0$ is sufficiently small. Then we have
\begin{align}\label{ShatS}
% \begin{split}
    \mathbb{P} (S \neq \hat{S}) &= \mathbb{P} ( \exists i \in \mathbb{N} : \eta (A_i) \geq 2 ) \nonumber\\
    &\leq \sum_{i=1}^\infty \mathbb{P} ( \eta (A_i) \geq 2 ) \nonumber\\
...
\begin{itemize}
    ...
    \item Quantitative Normal approximation (Wasserstein, Kolmogorov distance), see \cite{ReitznerSchulte13},\cite{schulte}, \cite{eichelsbacher}
...
% \bibitem{bonnet}
% G.~Bonnet and A.~Gusakova, \emph{{Concentration inequalities for Poisson $ U $-statistics}}, arXiv preprint arXiv:2404.16756 (2024).

% \bibitem{dst}
% L.~Decreusefond, M.~Schulte, and C.~Th{\"a}le, \emph{{Functional Poisson approximation in Kantorovich--Rubinstein distance with applications to U-statistics and stochastic geometry}}, The Annals of Probability \textbf{44} (2016), no.~3, 2147--2197.

% \bibitem{eichelsbacher}
% P.~Eichelsbacher and C.~Th{\"a}le, \emph{{New Berry-Esseen bounds for non-linear functionals of Poisson random measures}}, Electronic Journal of Probability \textbf{19} (2014), 1--25.


% \bibitem{pianoforte}
% F.~Pianoforte and M.~Schulte, \emph{Poisson approximation with applications to stochastic geometry}, Electronic Journal of Probability \textbf{26} (2021), 1--36.

% \bibitem{reitznerschulte}
% M.~Reitzner and M.~Schulte, \emph{{Central limit theorems for U-statistics of Poisson point processes}}, The Annals of Probability \textbf{41} (2013), no.~6, 3879--3909.

% \bibitem{reitznerschultethaele}
% M.~Reitzner, M.~Schulte, and C.~Th{\"a}le, \emph{{Limit theory for the Gilbert graph}}, Advances in Applied Mathematics \textbf{88} (2017), 26--61.

% \bibitem{schulte}
% M.~Schulte, \emph{{Normal approximation of Poisson functionals in Kolmogorov distance}}, Journal of Theoretical Probability \textbf{29} (2016), 96--117.

% \bibitem{schulte2}
% M.~Schulte and C.~Th{\"a}le, \emph{{Moderate deviations on Poisson chaos}}, Electronic Journal of Probability \textbf{29} (2024), 1--27.
